...
import { markExtractedFieldAbsentService } from "@/lib/documents/extractedFieldsService";
...
export const POST = withAuth(
  async (_req: NextRequest, auth, tx, { params }: { params: Promise<{ documentId: string; fieldDefinitionId: string }> }) => {
    ...
    try {
      const updated = await markExtractedFieldAbsentService(tx, auth, documentId, fieldDefinitionId);
