...
import { addRuleToSchemeService, SchemeServiceError, schemeErrorStatus } from "@/lib/schemes/service";
import { addSchemeRuleSchema } from "@/lib/validation/scheme";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ schemeId: string }> }) => {
    ...
    const decision = can(auth, "scheme.addRule", found.resource);
    if (!decision.allowed) return Response.json({ error: "forbidden" }, { status: decision.status });

    const body = await req.json().catch(() => null);
    const parsed = addSchemeRuleSchema.safeParse(body);
    if (!parsed.success) {
      return Response.json({ error: "invalid_input", issues: parsed.error.issues }, { status: 400 });
    }

    try {
      const updated = await addRuleToSchemeService(
        tx,
        auth,
        schemeId,
