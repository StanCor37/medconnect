...
import { shareWithClientService, caseErrorStatus, CaseServiceError } from "@/lib/cases/service";
import { shareWithClientSchema } from "@/lib/validation/case";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ caseId: string }> }) => {
    ...
    const decision = can(auth, "case.shareWithClient", found.resource);
    if (!decision.allowed) return Response.json({ error: "forbidden" }, { status: decision.status });

    const body = await req.json().catch(() => null);
    const parsed = shareWithClientSchema.safeParse(body);
    if (!parsed.success) {
      return Response.json({ error: "invalid_input", issues: parsed.error.issues }, { status: 400 });
    }

    try {
      const updated = await shareWithClientService(tx, auth, caseId, parsed.data.clientId, parsed.data.version);
