...
import { submitCaseService, caseErrorStatus, CaseServiceError } from "@/lib/cases/service";
import { submitCaseSchema } from "@/lib/validation/caseLifecycle";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ caseId: string }> }) => {
    ...
    const decision = can(auth, "case.submit", found.resource);
    ...
    try {
      const updated = await submitCaseService(tx, auth, caseId, parsed.data);
