...
import { assignCaseService, caseErrorStatus, CaseServiceError } from "@/lib/cases/service";
import { assignCaseSchema } from "@/lib/validation/case";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ caseId: string }> }) => {
    ...
    const decision = can(auth, "case.assign", found.resource);
    ...
    try {
      const updated = await assignCaseService(tx, auth, caseId, parsed.data.assignedToUserId, parsed.data.version);
