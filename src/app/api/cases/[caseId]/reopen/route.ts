...
import { reopenCaseService, caseErrorStatus, CaseServiceError } from "@/lib/cases/service";
import { reopenCaseSchema } from "@/lib/validation/caseLifecycle";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ caseId: string }> }) => {
    ...
    const decision = can(auth, "case.reopen", found.resource);
    ...
    try {
      const updated = await reopenCaseService(tx, auth, caseId, parsed.data);
