...
import { cancelCaseService, caseErrorStatus, CaseServiceError } from "@/lib/cases/service";
import { cancelCaseSchema } from "@/lib/validation/caseLifecycle";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ caseId: string }> }) => {
    ...
    const decision = can(auth, "case.cancel", found.resource);
    ...
    try {
      const updated = await cancelCaseService(tx, auth, caseId, parsed.data);
