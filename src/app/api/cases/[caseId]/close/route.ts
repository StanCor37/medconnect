...
import { closeCaseService, caseErrorStatus, CaseServiceError } from "@/lib/cases/service";
import { closeCaseSchema } from "@/lib/validation/caseLifecycle";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ caseId: string }> }) => {
    ...
    const decision = can(auth, "case.close", found.resource);
    ...
    try {
      const updated = await closeCaseService(tx, auth, caseId, parsed.data);
