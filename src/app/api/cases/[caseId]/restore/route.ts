...
import { restoreCaseService, caseErrorStatus, CaseServiceError } from "@/lib/cases/service";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ caseId: string }> }) => {
    ...
    const decision = can(auth, "case.restore", found.resource);
    ...
    try {
      const updated = await restoreCaseService(tx, auth, caseId, parsed.data.version);
