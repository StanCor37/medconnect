...
import { archiveRuleService, RuleServiceError, ruleErrorStatus } from "@/lib/rules/service";
...
export const POST = withAuth(
  async (req: NextRequest, auth, tx, { params }: { params: Promise<{ ruleId: string }> }) => {
    ...
    const decision = can(auth, "rule.archive", found.resource);
    ...
    try {
      const updated = await archiveRuleService(tx, auth, ruleId, parsed.data.version);
